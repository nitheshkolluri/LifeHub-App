        ...
        <div className="flex items-center gap-4">
           ...
           <div className="flex bg-white/50 backdrop-blur-md p-1 rounded-full border border-white/50">
            {(['all', 'completed'] as const).map(f => (
                ...
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-1 text-[10px] font-bold rounded-full uppercase transition-all ${
                   ...
                   <div className="flex gap-3">
                      {(['low', 'medium', 'high'] as const).map(p => (
                        <button
                          ...
                          className={`flex-1 py-4 rounded-2xl text-xs font-black uppercase transition-all border-2 flex flex-col items-center gap-2 ${
                            priority === p 
